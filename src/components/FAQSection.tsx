    ...
    <section className="py-16 md:py-24 bg-white" id="faq">
      <div className="container mx-auto max-w-3xl px-4 sm:px-6 lg:px-16">
        <div className="text-center mb-10 md:mb-12">
          <h2 className="text-2xl md:text-3xl font-semibold mb-3 md:mb-4 text-[var(--color-on-surface)]">Patient FAQ</h2>
          <p className="text-[var(--color-on-surface-variant)] text-base md:text-lg">
            Clear answers regarding your visits, billing, and appointments at SIMS Anantapur.
          </p>
        </div>
          ...
          {faqs.map((faq, i) => (
            ...
            >
               <div className="flex items-center justify-between">
                 <h3 className="text-sm md:text-base font-semibold text-[var(--color-on-surface)] pr-4">{faq.q}</h3>
                 <ChevronDown className={`w-4 h-4 md:w-5 md:h-5 flex-shrink-0 text-[var(--color-on-surface-variant)] transition-transform ${open === i ? 'rotate-180' : ''}`} />
               </div>
               {open === i && (
